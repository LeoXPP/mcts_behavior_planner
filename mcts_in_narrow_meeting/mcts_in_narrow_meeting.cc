...
static bool SolveQuadraticEquationForRealRoots(const float a, const float b,
                                               const float c, float *root1,
                                               float *root2) {
...
void XICAMCTSFunction::InitNode(
  ...
  new_node->set_relative_time(parent_node->relative_time() +
                              mcts_param_.time_step[parent_node->iter()]);
...
void XICAMCTSFunction::ChooseAction(
    MCTSNode *node,
    std::unordered_map<std::string, VehicleAction> &selected_action) {
  if (!node) {
  ...
  //     selected_action[id] =
  //     mcts_param_.ego_agent_action.at(untried_actions_idx);
  //   }
...
bool XICAMCTSFunction::StateChange(
    MCTSNode *node,
    const std::unordered_map<std::string, VehicleAction> &action,
    MCTSNode *new_node) {
  if (!node || !new_node) {
  ...
  for (auto &[id, decision_type] : mcts_param_.decision_type) {
    ...
    if (decision_type == DecisionType::EgoSearch ||
      ...
      if (!XICAIDMModel(cur_state.at(id), next_state.at(id),
      ...
      point_idx =
          std::min(point_idx, static_cast<int>(mcts_param_.pred_obs.at(id)
                                                   .trajectory()[0]
                                                   .trajectory_point()
                                                   .size() -
                                               1));
      auto &obs_traj = mcts_param_.pred_obs.at(id)
                           .trajectory()[0]
                           .trajectory_point()[point_idx];
...
bool XICAMCTSFunction::XICAIDMModel(const VehicleState &cur_state,
                                    VehicleState &next_state, double dt,
                                    const VehicleState *leader_state) {
  XICAIDMParam idm_param = mcts_param_.xica_idm_param;
  double safe_dist =
      idm_param.idm_min_dist_ + idm_param.idm_desired_time_ * cur_state.vel();
  double dist = std::numeric_limits<double>::max();

  if (leader_state) {
    double delta_vel = std::fabs(cur_state.vel() - leader_state->vel());
    safe_dist += std::max(
        cur_state.vel() * delta_vel /
            (2.0 * std::sqrt(idm_param.max_acc_ * idm_param.comfort_acc_)),
        0.0);
  ...
  if (mcts_param_.ego_target_speed < 1) {
    ...
    // // AERROR << "xiepanpan Ego target speed is too small reset to " <<
    // ego_target_speed_;
  } else {
  ...
  double acc =
      idm_param.max_acc_ *
      (1.0 - std::pow(cur_state.vel() / ego_target_speed_, idm_param.acc_exp_) -
  ...
  PathPoint path_point =
      mcts_param_.obs_path.at("ego").Evaluate(next_state.s());
  ...
  while (node->iter() < mcts_param_.max_iter) {
    ...
    if (!mcts_param_.use_ref_pre_construct) {
      for (auto &[id, obstacle] : mcts_param_.pred_obs) {
        ...
        next_state.at(id).set_kappa(obs_traj.path_point().kappa());
        selected_action[id] =
            VehicleAction(obs_traj.da(), obs_traj.path_point().dkappa());
      }
...
void XICAMCTSFunction::Prepuring(MCTSNode *new_node) {
  ...
  for (auto &[id, decision_type] : mcts_param_.decision_type) {
    ...
    // // ATRACE << "xiepanpan: obstacle id is:" << id << "the decision type
    // is:" <<decision_type;
    if (decision_type == DecisionType::ObsSearch) {
      ...
      if (dis < 10.0) {
        apollo::common::math::Box2d ego_box(
            {next_state.at("ego").x(), next_state.at("ego").y()},
      ...
      for (auto &[cipv_id, cipv_decision_type] : mcts_param_.decision_type) {
        if (cipv_decision_type == DecisionType::ObsPrediction) {
          ...
          if (dis < 10.0) {
            apollo::common::math::Box2d cipv_box(
                {next_state.at(cipv_id).x(), next_state.at(cipv_id).y()},
                next_state.at(cipv_id).theta(),
...
bool XICAMCTSFunction::EgoSearchIDMModel(const VehicleAction &action,
  ...
  double new_theta = cur_state.theta() + cur_state.kappa() * ds +
                     0.5 * dkappa * current_velocity * dt * dt;
  double mid_theta =
      common::math::NormalizeAngle((cur_state.theta() + new_theta) / 2.0);
...
bool XICAMCTSFunction::XICAJerkModel(MCTSNode *node, const std::string &id,
  ...
  if (new_acc < mcts_param_.veh_param.min_acc) {
    new_acc = common::math::Clamp(new_acc, mcts_param_.veh_param.min_acc,
                                  mcts_param_.veh_param.max_acc);
  ...
  if (cur_state.vel() <= std::fabs(dt * mcts_param_.veh_param.min_acc)) {
    if (std::fabs(cur_state.vel()) < kEpsilon &&
      ...
      if (SolveQuadraticEquationForRealRoots(
              0.5f * new_jerk, new_acc, cur_state.vel(), &root1_, &root2_)) {
        for (const float root : {root1_, root2_}) {
          if (root > kEpsilon && root < dt) {
            stop_time = (new_jerk > kEpsilon) ? std::min(root, kMaxStopTime)
                                              : std::max(root, kMinStopTime);
          }
        }
      }
    }
  ...
  if (stop_time <= dt + kEpsilon && stop_time >= -kEpsilon) {
    ds = cur_state.vel() * stop_time +
         0.5f * cur_state.acc() * stop_time * stop_time +
         0.16667f * new_jerk * stop_time * stop_time * stop_time + 1.0;
    new_vel = 0.f;
    new_acc = 0.f;
    new_jerk = 0.f;
    new_theta = cur_state.theta() + cur_state.kappa() * ds +
                0.5f * new_dkappa * cur_state.vel() * stop_time * stop_time;
  } else {
    ...
    ds = cur_state.vel() * dt + 0.5f * cur_state.acc() * dt_sq +
         0.16667f * new_jerk * dt * dt_sq;
    new_theta = cur_state.theta() + cur_state.kappa() * ds +
                0.5f * new_dkappa * cur_state.vel() * dt_sq;
  }
...
double XICAMCTSFunction::RewardFun(MCTSNode *node) {
  ...
  double total_r = 0.0;
  // double time_factor =
  //     std::min(mcts_param_.xica_reward_info.w_cons_act,
  //     mcts_param_.xica_reward_info.w_pred) / mcts_param_.max_iter;
  ...
  for (const auto &[id, decision_type] : mcts_param_.decision_type) {
    ...
    if (decision_type == DecisionType::EgoSearch) {
      ...
      double other_w = 1.0;
      reward_details.safety_reward += other_w *
                                      mcts_param_.xica_reward_info.xica_w_safe *
                                      XICASafetyReward(cur_state);
      reward_details.idm_velocity_reward +=
          other_w * mcts_param_.xica_reward_info.xica_w_eff *
          XICAIDMVelocityReward(cur_state.at(id));
      ...
      const auto &trajectory_iter =
          mcts_param_.last_modified_trajectories.find(id);
      if (trajectory_iter != mcts_param_.last_modified_trajectories.end() &&
          !trajectory_iter->second.trajectory_point().empty()) {
        reward_details.history_consistency_reward +=
            mcts_param_.xica_reward_info.xica_w_cons_his *
            XICAHistoryConsistencyReward(cur_state.at(id),
                                         node->relative_time(),
                                         trajectory_iter->second);
      }

      // Prediction reward
      if (node->node_type() == NodeType::PRED ||
          node->node_type() == NodeType::NEWPRED) {
        reward_details.prediction_reward += mcts_param_.xica_reward_info.w_pred;
      } else {
        reward_details.prediction_reward +=
            (mcts_param_.xica_reward_info.w_pred) *
            XICAPredictionReward(cur_state.at(id), node->relative_time(),
                                 mcts_param_.pred_obs.at(id));

        // Acto+n consistency reward
        if (node->iter() >= 2) {
          reward_details.action_consistency_reward +=
              mcts_param_.xica_reward_info.w_cons_act *
              ActionConsistencyReward_(
                  node->selected_action().at(id),
                  node->parent()->selected_action().at(id));
        }
      }
      ...
      reward_details.xica_efficiency_reward +=
          other_w * mcts_param_.xica_reward_info.xica_w_eff *
          XICAEfficiencyReward(cur_state.at(id), id);
      reward_details.acc_reward += other_w *
                                   mcts_param_.xica_reward_info.xica_w_acc *
                                   AccReward(cur_state.at(id), id);
      reward_details.safety_reward += other_w *
                                      mcts_param_.xica_reward_info.xica_w_safe *
                                      XICASafetyReward(cur_state);
      reward_details.occupancy_reward +=
      ...
      reward_details.state_cons_reward +=
          other_w * mcts_param_.xica_reward_info.w_eff *
          StateConsistencyReward(cur_state.at(id), node->relative_time(),
                                 mcts_param_.pred_obs.at(id));
    }
    if (decision_type == DecisionType::ObsSearch) {
...
double XICAMCTSFunction::XICASafetyReward(
  ...
  for (const auto &[id, agent_state] : cur_state) {
    if (id == "ego" ||
        mcts_param_.decision_type.at(id) != DecisionType::ObsSearch) {
      continue;
    }
    ...
    double normalized_reward =
        (longitudinal_distance - longitudinal_threshold_low) /
...
double
XICAMCTSFunction::XICAPredictionReward(const VehicleState &veh_state,
                                       const double cur_t,
  ...
  point_idx = std::min(point_idx, static_cast<int>(trajectory.size()) -
                                      1); // Clamp to valid index
  ...
  double distance =
      std::hypot(veh_state.x() - target_x, veh_state.y() - target_y);
  double reward = 0.0;
  ...
  if (distance <= DIST_THRESHOLD_MIN) {
    ...
    reward = 1.0 - (distance - DIST_THRESHOLD_MIN) /
                       (DIST_THRESHOLD_MAX - DIST_THRESHOLD_MIN);
  }
  // Ensure the reward is within [0.0, 1.0]
  reward = std::min(std::max(reward, 0.0), 1.0);
  return reward;
}

double XICAMCTSFunction::XICAReflineReward(
    const std::unordered_map<std::string, VehicleState> &cur_state,
    const std::string &id) {
...
double XICAMCTSFunction::XICAHistoryConsistencyReward(
    const VehicleState &veh_state, const double cur_t,
  ...
  int cur_point_idx = std::min(
      static_cast<int>(cur_t / 0.2),
      static_cast<int>(last_modified_traj.trajectory_point().size() - 1));
  ...
  distance_reward = std::min(std::max(distance_reward, 0.0), 1.0);
  velocity_reward = std::min(std::max(velocity_reward, 0.0), 1.0);
  acceleration_reward = std::min(std::max(acceleration_reward, 0.0), 1.0);

  // Combine rewards with weights
  double reward = weight_distance * distance_reward +
                  weight_velocity * velocity_reward +
                  weight_acceleration * acceleration_reward;
...
double XICAMCTSFunction::SmoothnessReward(const VehicleState &cur_state,
  ...
  if (acc_change > mcts_param_.veh_param.comfort_jerk) {
    ...
    double R_smooth =
        (1 - acc_change / (mcts_param_.veh_param.comfort_jerk + 1e-5));
...
XICAMCTSFunction::StateConsistencyReward(const VehicleState &cur_state,
  ...
  if (!pred_obs.trajectory().empty() &&
      !pred_obs.trajectory()[0].trajectory_point().empty()) {
    int point_idx =
        std::min(static_cast<int>(cur_t / 0.1),
                 static_cast<int>(
                     pred_obs.trajectory()[0].trajectory_point().size() - 1));

    const auto &traj_point =
        pred_obs.trajectory()[0].trajectory_point()[point_idx];
...
double XICAMCTSFunction::TargetSpeedRward(const VehicleState &veh_state,
                                          const std::string &id) {
  double target_speed;
  if (mcts_param_.obs_target_speed.find(id) ==
      mcts_param_.obs_target_speed.end()) {
...
double XICAMCTSFunction::AccReward(const VehicleState &veh_state,
  ...
  double acc_alignment =
      (acceleration * acc_direction) / mcts_param_.veh_param.max_acc;
  if (acc_alignment < 0) {
    return 0;
  } else {
...
bool XICAMCTSFunction::AccModel(const VehicleAction &action,
                                const VehicleState &cur_state,
                                VehicleState &next_state, const double dt,
                                const std::string id) {
  // Linear acc model
