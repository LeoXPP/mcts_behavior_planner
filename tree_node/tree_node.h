...
struct VehicleRewardDetails {
  ...
  double total_reward() const {
    return xica_efficiency_reward + acc_reward + safety_reward + idm_velocity_reward + history_consistency_reward +
           prediction_reward + action_consistency_reward + occupancy_reward + refline_reward + state_cons_reward;
  }
};
...
class MCTSNode {
 public:
  MCTSNode() = default;
  MCTSNode(int iter, std::unordered_map<std::string, VehicleState> &state, MCTSNode *parent = nullptr)
 ...
 public:
  ...
  inline const std::unordered_map<std::string, VehicleState> &state() const { return cur_state_; }
  inline const std::unordered_map<std::string, VehicleAction> &selected_action() const { return selected_action_; }
  inline const std::unordered_map<std::string, VehicleReward> &agent_reward() const { return agent_reward_; }
  ...
  inline std::unordered_map<std::string, VehicleState> &get_state() { return cur_state_; }
  inline std::unordered_map<std::string, VehicleReward> &get_agent_reward() { return agent_reward_; }
  ...
  inline void set_state(const std::unordered_map<std::string, VehicleState> &state) { cur_state_ = state; }
  inline void set_parent(MCTSNode *parent) { parent_ = parent; }
  inline void set_action(const std::unordered_map<std::string, VehicleAction> &action) { selected_action_ = action; }
  inline void set_static_reward(double reward) { static_reward_ = reward; }
  inline void set_relative_time(double relative_time) { relative_time_ = relative_time; }
  ...
  inline void set_expanded_num(int expanded_num) { expanded_num_ = expanded_num; }
  inline void set_node_type(NodeType node_type) { node_type_ = node_type; }
  inline void set_untried_actions_idx(int untried_actions_idx) { untried_actions_idx_ = untried_actions_idx; }
  inline void set_agent_reward_by_id(const std::string &id, const VehicleReward &reward) { agent_reward_[id] = reward; }
