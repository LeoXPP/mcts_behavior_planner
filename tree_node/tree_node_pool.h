...
public:
  ...
  TreeNodePool(int pool_size, const std::unordered_map<std::string, VehicleState> &init_state)
      : pool_size_(pool_size) {
    pool_.reserve(pool_size_);
    // free_nodes_ 用于维护当前可用的节点列表
    for (int i = 0; i < pool_size_; ++i) {
      // 注意：这里调用 MCTSNode 的构造函数，传入初始状态和 nullptr 作为父节点
      MCTSNode *node = new MCTSNode(0, const_cast<std::unordered_map<std::string, VehicleState>&>(init_state), nullptr);
